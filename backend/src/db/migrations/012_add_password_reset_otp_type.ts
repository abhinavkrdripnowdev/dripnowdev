...
export async function up(knex: Knex): Promise<void> {
  ...
  if (isSqlite) {
    ...
    await knex.schema.createTable('verification_tokens', (table) => {
      ...
      table
        .enu('type', ['phone_otp', 'email_otp', 'email_token', 'password_reset_otp'], {
          useNative: true,
          enumName: 'verification_type_enum',
        })
        .notNullable();
      table.string('token', 512).notNullable().comment('Hashed OTP or verification token');
    ...
    await knex.raw("ALTER TABLE verification_tokens MODIFY COLUMN type ENUM('phone_otp', 'email_otp', 'email_token', 'password_reset_otp') NOT NULL;");
  }
}
